<template>
    <div class="searchBar">   
        <el-form :inline="true" :model="searchUserForm" class="demo-form-inline">
            ...
            <el-form-item label="角色">
                <el-select v-model="searchUserForm.roleId" clearable placeholder="选择级别" style="width: 100px;">
                    <el-option
                        key=1
                        label="管理员"
                        :value=1
            ...
            <el-form-item label="教师工号">
                <el-input v-model.number="searchUserForm.userno"    clearable placeholder="教师工号" style="width: 100px;"/>
            </el-form-item>
            <el-form-item label="姓名">
                <el-input v-model="searchUserForm.username" clearable placeholder="姓名" style="width: 100px;"/>
            </el-form-item>
    ...
    <div>
        <el-table :data="tableData" stripe style="width: 100%" >
            ...
            <el-table-column prop="userno" label="教师工号"  />
            <el-table-column prop="username" label="姓名" />
            <el-table-column prop="sex" label="性别" :formatter="sexFormateter"/>
            <el-table-column prop="roleId" label="角色" :formatter="roleIdFormateter"/>
            <el-table-column prop="gradeId" label="班级" :formatter="gradeIdFormateter"/>
            <el-table-column prop="classId" label="班级" :formatter="classIdFormateter"/>
...
<script lang="ts" setup>
    ...
    import { getSeachUser } from '@/api/user'
    ...
    const searchUserForm = reactive<paramsUser>({
        userno:null,
        username:'',
        sex:null,
        roleId:null,
    ...
    const onSubmit = async() => {
        try {
            ...
            const seachUsers:any =await getSeachUser(searchUserForm);
            console.log(seachUsers.data.users,'seachUsers')
    ...
    const roleIdFormateter = (row:any)=>{
        if(row.roleId == 1){
            return "管理员"
    ...
    const classIdFormateter = (row:any)=>{
        if(row.classId == 1){
            return '设开班'
    ...
    const gradeIdFormateter = (row:any)=>{
        if(row.gradeId == 1){
            return "22级"
        }else if(row.gradeId == 2){
            return "21级"
        }else if(row.gradeId == 3){
            return "20级"
        }else if(row.gradeId == 4){
            return "19级"
        }
        return ""
    }
